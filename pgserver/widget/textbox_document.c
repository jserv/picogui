/* $Id: textbox_document.c,v 1.26 2002/01/29 02:52:35 lonetech Exp $
 *
...
g_error text_insert_linebreak(struct textbox_cursor *c) {
  ...
  if (c->c_line) {
    ...
    errorcheck;
    newline->flags = c->c_line->flags |DIVNODE_NEED_REDRAW|DIVNODE_NEED_RECALC|
      DIVNODE_PROPAGATE_RECALC|DIVNODE_PROPAGATE_REDRAW;
    newline->split = c->c_line->split;
    c->widget->dt->flags |= DIVTREE_NEED_REDRAW|DIVTREE_NEED_RECALC;
...
g_error text_insert_string(struct textbox_cursor *c, const char *str,
  ...
  c->c_div->flags |= DIVNODE_NEED_REDRAW|DIVNODE_NEED_RECALC|
    DIVNODE_PROPAGATE_RECALC|DIVNODE_PROPAGATE_REDRAW;
  c->widget->dt->flags |= DIVTREE_NEED_REDRAW|DIVTREE_NEED_RECALC;
